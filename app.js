...
function openDetail(type, id) {
  ...
  if (type === 'poi') {
    ...
    sidebar = `<a class="button primary wide" href="${mapsLink(item.query)}">In Google Maps öffnen</a>${renderLinks(item.links || [])}`;
  }
  ...
  if (type === 'food') {
    ...
    sidebar = item.query ? `<a class="button primary wide" href="${mapsLink(item.query)}">In Google Maps suchen</a>` : '';
  }

  $('#deepDetail').innerHTML = `<div class="detail-title"><p class="section-kicker">${eyebrow}</p><h2>${title}</h2></div><div class="deep-grid"><article>${body}</article><aside>${sidebar}<div class="mini-card"><h3>Vor Ort prüfen</h3><ul><li>Öffnungszeiten</li><li>Parken</li><li>Reservierung</li><li>Hitze / Wetter</li></ul></div></aside></div>`;
  location.hash = 'details';
}

function renderLinks(links) {
  if (!links.length) return '';
  return `<div class="mini-card"><h3>Links</h3>${links.map(link => `<a href="${mapsLink(link.query)}">${link.label}</a>`).join('')}</div>`;
}
...
function relatedForDay(day) {
  ...
  return `<div class="mini-card"><h3>Passende Orte</h3>${pois.map(p => `<a href="${mapsLink(p.query)}">${p.name}</a>`).join('')}</div>`;
}
...
function setupDetailClicks() {
  $$('[data-detail-type]').forEach(btn => btn.addEventListener('click', () => openDetail(btn.dataset.detailType, btn.dataset.detailId)));
}
...
function renderDayFilters() {
  ...
  $$('[data-day-filter]').forEach(button => button.addEventListener('click', () => { activeDayFilter = button.dataset.dayFilter; renderDayFilters(); renderDays(); setupDetailClicks(); }));
}
function renderDays() {
...
function renderPoiToolbar() {
  ...
  $$('[data-poi-filter]').forEach(button => button.addEventListener('click', () => { activePoiFilter = button.dataset.poiFilter; renderPoiToolbar(); renderPois(); setupDetailClicks(); }));
}
function renderPois() {
  const filtered = activePoiFilter === 'Alle' ? data.pois : data.pois.filter(poi => poi.type === activePoiFilter);
  $('#poiGrid').innerHTML = filtered.map(poi => { const i = data.pois.indexOf(poi); return `<article class="poi-card"><span class="poi-type">${poi.type} · ${poi.place}</span><h3>${poi.name}</h3><p>${poi.text}</p><div class="inline-actions">${detailButton('poi', i)}<a href="${mapsLink(poi.query)}">Maps</a></div></article>`; }).join('');
}
function renderFood() {
...
function setupJournal() {
  ...
  function load() { textarea.value = localStorage.getItem(`journal-${select.value}`) || ''; hint.textContent = ''; }
  select.addEventListener('change', load);
  $('#journalForm').addEventListener('submit', event => { event.preventDefault(); localStorage.setItem(`journal-${select.value}`, textarea.value.trim()); hint.textContent = 'Gespeichert auf diesem Gerät.'; });
  load();
}
function setupMenu() {
...
renderRoute(); renderDestinations(); renderDayFilters(); renderDays(); renderPoiToolbar(); renderPois(); renderFood(); renderKids(); renderPacking(); setupJournal(); setupMenu(); setupDetailClicks();
